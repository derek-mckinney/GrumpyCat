...
###   grumpycatcore.org and digibyte.org and compares them.
...
WORKINGDIR="/tmp/grumpycat_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://grumpycatcore.org"
HOST2="https://grumpycat.io"
BASEDIR="/bin/"
VERSIONPREFIX="grumpycat-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'grumpycat-',
...
if [ $? -ne 0 ]; then
   echo "grumpycat.org failed to provide signature file, but grumpycatcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "grumpycat.org and grumpycatcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the GrumpyCat Core binary release signing key installed?"
   fi
